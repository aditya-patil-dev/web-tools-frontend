...
const JpgToPngTool = () => {
  ...
  const processFiles = (files: File[]) => {
    ...
    const validFiles = files.filter((file) => file.type === ACCEPTED_TYPE);

    if (validFiles.length !== files.length) {
      toast.warning("Only JPG images are allowed.", "Invalid File Type");
    }
  ...
  const handleConvert = async () => {
    if (!selectedFiles.length) {
      toast.error("Please upload JPG files first.", "No Files Selected");
      return;
    }
  ...
  const resetForm = () => {
    ...
    setPreviews([]);
    setConvertProgress(0);
    const input = document.getElementById("inputJpg") as HTMLInputElement;
  ...
  return (
    ...
    >
      ...
      <div className="tool-info-banner">
        <FiCheckCircle />
        <p>
          Convert up to {MAX_FILES} JPG images to PNG format &mdash; all processing
      ...
      >
        ...
        <label htmlFor="inputJpg" className="tool-upload-label">
          <FiUpload className="upload-icon" />
          <h3>Drop JPG files here</h3>
      ...
      <AnimatePresence>
        {convertedFiles.length > 0 && (
          ...
          >
              ...
              {convertedFiles.map((file, i) => (
                ...
                >
                  <img
                    ...
                    alt={`Converted ${i + 1}`}
                  />
                  <div className="preview-badge">PNG</div>
            ...
            >
              <FiDownload />
              Download {convertedFiles.length > 1 ? "as ZIP" : "PNG"}
            </motion.button>
          </motion.div>
        )}
      </AnimatePresence>
