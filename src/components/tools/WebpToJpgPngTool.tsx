...
const WebpToJpgPngTool = () => {
    ...
    const handleConvert = async () => {
        if (!selectedFiles.length) {
            toast.error("Please upload WebP files first.", "No Files Selected");
            return;
        }
        ...
        try {
            ...
            toast.success(
                `${converted.length} image(s) converted to ${outputFormat.toUpperCase()}!`,
                "Conversion Complete"
            );
        } catch (err) {
            console.error(err);
    ...
    const handleDownload = async () => {
        ...
        try {
            if (convertedFiles.length === 1) {
                ...
                saveAs(blob, `converted-images-${outputFormat}.zip`);
    ...
    const resetForm = () => {
        ...
        setPreviews([]);
        setConvertProgress(0);
        const input = document.getElementById("inputWebp") as HTMLInputElement;
                ...
                {convertedFiles.length > 0 && (
                    ...
                    >
                        <div className="result-header">
                            ...
                            <p>{convertedFiles.length} image(s) converted to {outputFormat.toUpperCase()}</p>
                        </div>
                            ...
                            {convertedFiles.map((file, i) => (
                                ...
                                >
                                    <img
                                        ...
                                        alt={`Converted ${i + 1}`}
                                    />
                                    <div className="preview-badge">{outputFormat.toUpperCase()}</div>
                        ...
                        >
                            <FiDownload />
                            Download {convertedFiles.length > 1 ? "as ZIP" : outputFormat.toUpperCase()}
                        </motion.button>
                    </motion.div>
                )}
            </AnimatePresence>
...
export default WebpToJpgPngTool;