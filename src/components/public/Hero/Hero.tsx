...
const badgeVariants = {
    ...
    visible: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.6, ease: [0.34, 1.56, 0.64, 1] as const } },
};
const titleVariants = {
    hidden: { opacity: 0, y: 30 },
    visible: { opacity: 1, y: 0, transition: { duration: 0.8, ease: [0.34, 1.56, 0.64, 1] as const, delay: 0.1 } },
};
const subtitleVariants = {
    hidden: { opacity: 0, y: 20 },
    visible: { opacity: 1, y: 0, transition: { duration: 0.7, ease: "easeOut" as const, delay: 0.3 } },
};
const ctaContainerVariants = {
    hidden: { opacity: 0, y: 20 },
    visible: { opacity: 1, y: 0, transition: { duration: 0.6, delay: 0.5, staggerChildren: 0.1 } },
};
const ctaVariants = {
...
const trustVariants = {
    hidden: { opacity: 0 },
    visible: { opacity: 1, transition: { duration: 0.8, delay: 0.7, staggerChildren: 0.1 } },
};
...
export default function Hero({ config = DEFAULT_HERO_CONFIG }: HeroProps) {
    return (
        <section className="hero-wrapper">
            {/* Decorative blobs */}
            <motion.div className="hero-blob hero-blob-a"
                initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 0.6, scale: 1 }}
                transition={{ duration: 1.5, ease: "easeOut" }} />
            <motion.div className="hero-blob hero-blob-b"
                initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 0.6, scale: 1 }}
            ...
            <div className="hero-container">
                ...
                <motion.p className="hero-subtitle"
                    initial="hidden" animate="visible" variants={subtitleVariants}>
                    {config.subtitle}
                </motion.p>
                ...
                <motion.div className="hero-trust"
                    initial="hidden" animate="visible" variants={trustVariants}>
