...
export default function ToolPageFormPage() {
  ...
  const { toolPage, isLoading: isFetching } = useToolPageDetail(
    isEditMode ? slug : null,
  );
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof ToolPageFormData, string>>
  >({});
  ...
  useEffect(() => {
    if (toolPage && isEditMode) {
      ...
      const parsedFeatures = parseJsonArray<FeatureItem>(toolPage.features, [
        defaultFeature(),
      ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const rawSchema = formData.schema_markup;
    const schemaValue =
      !rawSchema || rawSchema === ""
        ? null
        : typeof rawSchema === "string"
          ? rawSchema.trim() || null // guard empty string
    ...
    loading.show({
      message: isEditMode ? "Updating tool page…" : "Creating tool page…",
    });
    ...
    <div className="adminPage">
      <Form onSubmit={handleSubmit}>
        ...
        >
          ...
          >
            Full article content with rich formatting (headings, lists, links,
            etc.)
          </p>
        </FormSection>
          ...
          <Textarea
            ...
            onChange={(e) => {
              try {
                ...
                setFormData((prev) => ({
                  ...prev,
                  schema_markup: e.target.value as any,
                }));
              }
            }}
            minRows={8}
