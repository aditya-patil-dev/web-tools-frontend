...
export const fetchPageComponents = cache(
  async (pageKey: string): Promise<PageComponentDTO[]> => {
    try {
...
export async function fetchAllComponents(filters?: ComponentFilters): Promise<{
  ...
  try {
    ...
    return { components: res.data, pagination: res.pagination };
  } catch (error) {
